...
- **Windows only.** The single-instance guard, tray icon, and lock file all rely on Win32 APIs.
- **Delete always goes through the Recycle Bin, never a permanent unlink.** This is a deliberate trust
  feature, not an oversight: every `os.remove`/`os.unlink`/`shutil.rmtree`/`Path.unlink` call is armed
  to fail in the test suite (see [Development](#development)), so a permanent-delete code path can
  never pass CI even if one were added by mistake.
